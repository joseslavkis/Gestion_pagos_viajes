...
class PaymentRestControllerTest extends ControllerIntegrationTestSupport {
    ...
    void legacyPendingReview_usesPersistedRateAndLabelsReconciliationWithoutProviderCall() throws Exception {
        ...
        paymentService.voidPayment(legacy.submissionId(), "admin@test.com");
        paymentSubmissionRepository.flush();
        entityManager.clear();
        PaymentSubmission voided = paymentSubmissionRepository.findByIdWithContext(legacy.submissionId()).orElseThrow();
        assertThat(voided.getStatus()).isEqualTo(PaymentSubmissionStatus.VOIDED);
        assertThat(voided.getOutcomes()).extracting(PaymentOutcome::getStatus)
                .containsExactlyInAnyOrder(PaymentOutcomeStatus.APPROVED, PaymentOutcomeStatus.VOIDED);
        assertThat(voided.getOutcomes().stream()
                .filter(outcome -> outcome.getStatus() == PaymentOutcomeStatus.APPROVED)
                .findFirst().orElseThrow().getAmountInTripCurrency()).isEqualByComparingTo("0.01");
    ...
    @Test
    void eightDecimalSnapshotSurvivesPreviewRegistrationReviewHistoryAndVoid() throws Exception {
        ExchangeRateQuote quote = new ExchangeRateQuote(new BigDecimal("1234.56789012"),
                HISTORICAL_PAYMENT_DATE, HISTORICAL_PAYMENT_DATE, "official", "provider-a", "2020-01-15T12:00:00Z");
        RegisteredSnapshot registered = registerQuotedSnapshot("eight-decimal-quote", "200000.00", "100.00", quote);
        ...
        assertThat(history).singleElement().satisfies(item ->
                assertThat(item.exchangeRate()).isEqualByComparingTo(quote.sellRate()));
        paymentService.voidPayment(registered.submissionId(), "admin@test.com");
        paymentSubmissionRepository.flush();
        entityManager.clear();
        assertPersistedSnapshot(registered.submissionId(), quote, PaymentSubmissionStatus.VOIDED);
        PaymentSubmission voided = paymentSubmissionRepository.findByIdWithContext(registered.submissionId()).orElseThrow();
        assertThat(voided.getOutcomes()).extracting(PaymentOutcome::getAmountInTripCurrency)
                .containsOnly(new BigDecimal("0.08"));
        assertThat(installmentRepository.findById(registered.installmentId()).orElseThrow().getPaidAmount())
                .isEqualByComparingTo("0.00");
        org.mockito.Mockito.verifyNoInteractions(exchangeRateService);
    }
    ...
    @Test
    void divergentLegacyFullApprovalFailsClosedWithoutChangingPersistedState() throws Exception {
        LegacyPendingFixture legacy = createDivergentLegacyPendingFixture("legacy-divergent-full");
        org.mockito.Mockito.clearInvocations(exchangeRateService);
    ...
    @Test
    void divergentLegacyPartialApprovalFailsClosedWithoutChangingPersistedState() throws Exception {
        LegacyPendingFixture legacy = createDivergentLegacyPendingFixture("legacy-divergent-partial");
        org.mockito.Mockito.clearInvocations(exchangeRateService);
    ...
    @Test
    void divergentLegacyFullRejectionPreservesOriginalSnapshotWithoutProvider() throws Exception {
        LegacyPendingFixture legacy = createDivergentLegacyPendingFixture("legacy-divergent-reject");
        org.mockito.Mockito.clearInvocations(exchangeRateService);
        ...
        assertThat(submission.getAmountInTripCurrency()).isEqualByComparingTo("1234567.00");
        assertThat(submission.getOutcomes()).singleElement().satisfies(outcome -> {
            assertThat(outcome.getStatus()).isEqualTo(PaymentOutcomeStatus.REJECTED);
            assertThat(outcome.getAmountInTripCurrency()).isEqualByComparingTo("1234567.00");
        });
        assertThat(installmentRepository.findById(legacy.installmentId()).orElseThrow().getPaidAmount())
                .isEqualByComparingTo("0.00");
        org.mockito.Mockito.verifyNoInteractions(exchangeRateService);
    ...
    private LegacyPendingFixture createDivergentLegacyPendingFixture(String prefix) throws Exception {
        ...
        Installment installment = createInstallment(fixture.trip(), fixture.user(), fixture.student(),
                1, "2000000.00", InstallmentStatus.YELLOW);
        PaymentSubmission submission = buildPendingSubmission(installment, createBankAccount(Currency.USD),
                "1000.00", "1234567.00", "inline-legacy-receipt");
        submission.setExchangeRate(new BigDecimal("1234.57"));
        ...
        entityManager.clear();
        return new LegacyPendingFixture(id, installment.getId());
    }

    private void assertDivergentLegacyUnchanged(LegacyPendingFixture legacy) {
        entityManager.clear();
        PaymentSubmission submission = paymentSubmissionRepository.findByIdWithContext(legacy.submissionId()).orElseThrow();
        assertThat(submission.getStatus()).isEqualTo(PaymentSubmissionStatus.PENDING);
        assertThat(submission.getAmountInTripCurrency()).isEqualByComparingTo("1234567.00");
        assertThat(submission.getExchangeRate()).isEqualByComparingTo("1234.57");
        assertThat(submission.getOutcomes()).isEmpty();
